     ...
     function create_json_all() {
        ...
        if (!empty($posts)) {
            ...
            foreach ($posts->result_array() as $post) {
                $data_produk='';
				$data_produk .='<div class="container" style="width:100%"><a href="'.base_url().'agenda/detail/'.$post['id'].'" ><label style="font-size:18px"><strong>'.$post['judul'].'</strong></label><i class="fa fa-calendar"></i>'.substr($post['tanggal'],6,2).'/'.substr($post['tanggal'],4,2).'/'.substr($post['tanggal'],0,4).' - '. $post['jam'].'</a><br><i class="fa fa-map-o"></i> '.$post['tempat'].'<br>'.$post['isi'].'<br><a href="'.base_url().'agenda/detail/'.$post['id'].'" class="btn btn-primary" style="color:#fff;margin-top:5px;">Selengkapnya</a></div><br>';
