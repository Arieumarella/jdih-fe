...

{
...

    {
        ...
        $sql = "select ppj_peraturan.peraturan_id, 
                        ...
                        order by tanggal DESC, ppj_peraturan_category.order asc, 
                        CAST(LEFT (noperaturan,4) AS UNSIGNED) DESC, CAST(RIGHT (noperaturan,4) AS UNSIGNED) DESC  
        ...
        if ($total > 0) {
            ...
            $sql2 = "select ppj_peraturan.peraturan_id from  ppj_peraturan,ppj_peraturan_category where 	
                ...
                tanggal DESC, ppj_peraturan_category.order asc, CAST(LEFT (noperaturan,4) AS UNSIGNED) DESC, CAST(RIGHT (noperaturan,4) AS UNSIGNED) DESC 
...

    {
        ...
        $sql = "select ppj_peraturan.peraturan_id, 
                        ...
                        tanggal DESC, ppj_peraturan_category.order asc, 
                        CAST(LEFT (noperaturan,4) AS UNSIGNED) DESC, CAST(RIGHT (noperaturan,4) AS UNSIGNED) DESC
...

    {
        ...
        ORDER BY tanggal DESC, ppj_peraturan_category.order asc, CAST(LEFT (noperaturan,4) AS UNSIGNED) DESC, CAST(RIGHT (noperaturan,4) AS UNSIGNED) DESC
...

    {
        ...
        $select = 'a.tanggal,a.tipe_dokumen, a.peraturan_id, a.noperaturan, a.download_count, b.percategoryname';
    ...
    function cekfavorit($sessid,$peraturan_id){
        $posts=array();
        $this->db->select('peraturan_id');
        $this->db->from('tb_favorit');
        $this->db->where('session_id',$sessid);
        $this->db->where('peraturan_id',$peraturan_id);
        $sql=$this->db->get();
        $posts=array('total'=>$sql->num_rows());
        return $posts;
    }
    function get_favorit($sessid){
        $posts=array();
        $this->db->where('session_id',$sessid);
        $sql=$this->db->get('tb_favorit');
        foreach($sql->result_array() as $rw){
            $posts[]=array('peraturan_id'=>$rw['peraturan_id']);
        }
        return $posts;
    }
    function simpanFavorit($sessid,$peraturan_id,$tipe){
        $posts=array();
        if($tipe=='simpan'){
            $this->db->where('session_id',$sessid);
            $this->db->where('peraturan_id',$peraturan_id);
            $this->db->delete('tb_favorit');
            $data=array('peraturan_id'=>$peraturan_id,'session_id'=>$sessid);
            $this->db->insert('tb_favorit',$data);
    ...
    function cekDataLogin($sessid){
        $posts=array();
        $this->db->where('sessid',$sessid);
        $sql=$this->db->get('tb_data_login');
        if($sql->num_rows()>0){
            $posts=array('respon'=>'ok');
        }else{
    ...
    function getProfile($sessid){
        $posts=array();
        $this->db->where('sessid',$sessid);
        $sql=$this->db->get('tb_data_login');
        if($sql->num_rows()>0){
            $rw=$sql->row();
            $posts=array('respon'=>'ok','username'=>$rw->username,
    ...
    function getOutLogin($sessid){
        $posts=array();
        $this->db->where('sessid',$sessid);
        $sql=$this->db->delete('tb_data_login');
