...
class Mdl_home extends CI_Model {
    ...
    function get_awal_produk() {
        $tgl = date("Y") - 2;
        return $this->db->query("select status_mencabut_id,abstrak,katalog,peraturan_id,tentang,noperaturan,status,peraturan_category_id,tanggal,download_count from ppj_peraturan where kondisi='3' and substring(tanggal,1,4)>='" . $tgl . "' order by tgl_modifikasi DESC ")->result_array();
    }
