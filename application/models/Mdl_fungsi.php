...
class Mdl_fungsi extends CI_Model
{
    ...
    function get_detail_infografis($id)
    {
        return $this->db->query("select * from tb_infografis where id='" . $id . "'");
    }

    function get_detail_konsultasi_publik($id)
    {
        return $this->db->query("select * from tb_konsultasi_publik where id='" . $id . "'");
    }

    function get_konsultasi_publik_komentar($id)
    {
        $kueri = $this->db->query("select * from tb_konsultasi_publik_komentar where konsultasi_publik_id ='" . $id . "' and status = 1 and parent_id IS NULL")->result_array();
        return $kueri;
    }
    function get_balasan_komentar($id)
    {
        $kueri = $this->db->query("select * from tb_konsultasi_publik_komentar where parent_id ='" . $id . "' and status = 1")->result_array();
        return $kueri;
    }

    public function konsultasi_publik_insert_komentar($data)
    {
        $this->db->insert('tb_konsultasi_publik_komentar', $data);
    ...
    function produk_count_result()
    {
        ...
        //                             and ppj_peraturan.tipe_dokumen != '2' and ppj_peraturan.tipe_dokumen != '4'";
        // $kueri = $this->db->query($sql, array('3', '1', '1'));
        // return $total = $kueri->num_rows();

        $this->db->select('ppj_peraturan.peraturan_id');
        $this->db->from('ppj_peraturan');
        $this->db->join('ppj_peraturan_category', 'ppj_peraturan.peraturan_category_id = ppj_peraturan_category.peraturan_category_id');
        $this->db->where('ppj_peraturan.kondisi', '3');
        $this->db->where('(ppj_peraturan.sifat = "1" OR ppj_peraturan.sifat IS NULL)');
        $this->db->where('ppj_peraturan_category.percategorykondisi', '1');
        $this->db->where('ppj_peraturan.tipe_dokumen !=', '2');
        $this->db->where('ppj_peraturan.tipe_dokumen !=', '4');
    ...
    function produk_limit($limit, $start)
    {
        ...
        //                  order by SUBSTRING(tanggal,1,8) DESC,CAST(LEFT (noperaturan,4) AS UNSIGNED) DESC, CAST(RIGHT (noperaturan,4) AS UNSIGNED) DESC" . ($limit == 0 || $limit == "" ? "" : " limit $start,$limit");
        //     $kueri = $this->db->query($sql, array('1', '3', '1'));

        // return $kueri;

        $this->db->select('ppj_peraturan.peraturan_id, ppj_peraturan.peraturan_category_id, ppj_peraturan.judul, ppj_peraturan.tanggal,
           ppj_peraturan.view_count, ppj_peraturan.download_count, ppj_peraturan.tipe_dokumen,
           ppj_peraturan.peraturan_id, ppj_peraturan.dept_id,
           ppj_peraturan.status, ppj_peraturan.file_abstrak, ppj_peraturan.file_upload');
        $this->db->from('ppj_peraturan');
        $this->db->join('ppj_peraturan_category', 'ppj_peraturan.peraturan_category_id = ppj_peraturan_category.peraturan_category_id');
        $this->db->where('ppj_peraturan_category.percategorykondisi', '1');
        $this->db->where('ppj_peraturan.tipe_dokumen !=', '2');
        ...
        $this->db->where('ppj_peraturan.kondisi', '3');
        $this->db->where('(ppj_peraturan.sifat = "1" OR ppj_peraturan.sifat IS NULL)');
        $this->db->order_by('SUBSTRING(ppj_peraturan.tanggal, 1, 8) DESC, CAST(LEFT(ppj_peraturan.noperaturan, 4) AS UNSIGNED) DESC, CAST(RIGHT(ppj_peraturan.noperaturan, 4) AS UNSIGNED) DESC');
    ...
    function infografis_count($limit, $start)
    {
        $this->db->select("id,judul,isi,gambar_1,tgl_modifikasi");
        $this->db->order_by("tgl_modifikasi", "DESC");

        $this->db->limit($limit, $start);
        $kueri = $this->db->get("tb_infografis");
        return $total = $kueri->num_rows();
    }

    function infografis_count_result()
    {
        $total = 0;
        $kueri = $this->db->query("select id from tb_infografis");
        return $total = $kueri->num_rows();
    }

    function infografis_limit($limit, $start)
    {
        $this->db->select("id,judul,isi,gambar_1,tgl_modifikasi");
        $this->db->order_by("tgl_modifikasi", "DESC");
        $this->db->limit($limit, $start);
    ...
    // konsultasi publik
    function konsultasi_publik_count($limit, $start)
    {
        $date = date('Y-m-d');

        $this->db->select("*");

        $this->db->where('durasi >=', $date);
        $this->db->limit($limit, $start);
        $kueri = $this->db->get("tb_konsultasi_publik");
        return $total = $kueri->num_rows();
    }

    function konsultasi_publik_count_result()
    {
        $total = 0;
        $date = date('Y-m-d');
        $kueri = $this->db->query("select id from tb_konsultasi_publik where durasi >= '$date'");
        return $total = $kueri->num_rows();
    }

    function konsultasi_publik_limit($limit, $start)
    {
        $this->db->select("*");
        $date = date('Y-m-d');
        $this->db->where('durasi >=', $date);
    ...
    function cari_detail_count_result($tipe_dokumen, $peraturan_category_id, $noperaturan, $tahun, $judul, $status, $dept_id, $tag_id)
    {
        ...
        if (!empty(trim($tipe_dokumen ?? ""))) {
            $k = $k . " and ppj_peraturan.tipe_dokumen='" . $this->db->escape_str($tipe_dokumen) . "'";
        } else {
            $k = $k . " and ppj_peraturan.tipe_dokumen != '2' and ppj_peraturan.tipe_dokumen != '4'";
        }
    ...
    public function cari_detail_count_result_monografi()
    {
        $qry = "SELECT 
                peraturan_id, 
                a.peraturan_category_id,
                judul, 
                tanggal,
                view_count,
                download_count, 
                ...
                file_abstrak,
                file_upload
                FROM (SELECT * FROM ppj_peraturan WHERE tipe_dokumen=4 AND kondisi='2') AS a
                LEFT JOIN (SELECT * FROM ppj_peraturan_category) AS b ON a.peraturan_category_id=b.peraturan_category_id";


        $kueri = $this->db->query($qry, array('3', '1', '1'));
        return $total = $kueri->num_rows();
    }


    function cari_detail_limit_monografi($limit, $start, $tipe_dokumen, $peraturan_category_id, $noperaturan, $tahun, $judul, $status, $dept_id, $tag_id)
    {
        ...
        if ($limit == 0 or $limit == "") {
            ...
            $qry = "SELECT 
                    ...
                    file_upload
                    FROM (SELECT * FROM ppj_peraturan WHERE tipe_dokumen=2 AND kondisi='3') AS a
                    LEFT JOIN (SELECT * FROM ppj_peraturan_category) AS b ON a.peraturan_category_id=b.peraturan_category_id limit " . $this->db->escape_str($start) . "," . $this->db->escape_str($limit) . "";
    ...
    public function cari_detail_count_result_putusan()
    {
        $qry = "SELECT 
                peraturan_id, 
                a.peraturan_category_id,
                ...
                file_abstrak,
                file_upload
                FROM (SELECT * FROM ppj_peraturan WHERE tipe_dokumen=4 AND kondisi='3') AS a
                LEFT JOIN (SELECT * FROM ppj_peraturan_category) AS b ON a.peraturan_category_id=b.peraturan_category_id";


        $kueri = $this->db->query($qry);
        return $total = $kueri->num_rows();
    }


    public function cari_detail_count_result_simpel($idPeraturan)
    {

        $tipe = '';

        if ($idPeraturan == 5) {
            $tipe = "AND jenis_peraturan = 'UU'";
        }

        if ($idPeraturan == 1) {
            $tipe = "AND (jenis_peraturan = 'PP' OR jenis_peraturan = 'PERPRES')";
        }

        if ($idPeraturan == 2) {
            $tipe = "AND jenis_peraturan = 'PERMEN'";
        }

        if ($idPeraturan == 3) {
            $tipe = "AND (jenis_peraturan = 'RPP' OR jenis_peraturan = 'RPERPRES')";
        }

        if ($idPeraturan == 4) {
            $tipe = "AND jenis_peraturan = 'RPERMEN'";
        }

        $qry = "SELECT * FROM t_pengusulan_puu WHERE 1=1 $tipe";
    ...
    function cari_detail_limit_putusan($limit, $start)
    {
    ...
    function cari_detail_limit_simepel($limit, $start, $idPeraturan)
    {
        ...
        if ($limit == 0 or $limit == "") {
            ...
            $qry = "SELECT * FROM t_pengusulan_puu WHERE 1=1 $tipe limit " . $this->db->escape_str($start) . "," . $this->db->escape_str($limit) . "";
    ...
    {
        // $this->load->library('Kirim_email_barux');
        ...
        $mail->Username = 'jdihpupr@pu.go.id';
        $mail->Password = 'Birohukum20';
        // $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
        $mail->Port = 587;                                    // TCP port to connect to
        // $mail->show_php_errors = E_ALL & ~E_DEPRECATED;

        $mail->From     = 'jdihpupr@pu.go.id';
