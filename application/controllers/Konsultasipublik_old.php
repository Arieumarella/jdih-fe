    ...
    function create_json_all()
    {
        ...
        if (!empty($posts)) {
            ...
            foreach ($posts->result_array() as $post) {
                $data_produk = '';
                $data_produk .= '<div class="row"><div class="col-12"></div><div class="col-12"><a href="' . base_url() . 'konsultasipublik/detail/' . $post['id'] . '"><label style="font-size:18px"><strong>' . $post['judul'] . '</strong></label>' . substr($post['pokok_pikiran'], 0, 150) . '...</a><br><a href="' . base_url() . 'konsultasipublik/detail/' . $post['id'] . '" class="btn btn-primary" style="color:#fff;margin-top:5px;">Selengkapnya</a></div></div>';
