				...
				<div class="panel panel-default" style="border: 2px solid #D2D6DE;border-radius:5px;background-color:#fff;width:100%">
					...
					<div class="panel-heading" style="background-color: #D2D6DE;height:50px;border-bottom:3px solid #fcfcfc;">
						<label style="color: #000;margin-top:10px;margin-left:10px">PRODUK HUKUM TERBARU</label>
                         ...
                         <img src="<?php base_url(); ?>assets/img/core-img/icon2.png" style="width:20px;height:20px">
                            <span class="translatable">Tidak Berlaku</span>
                            <img src="<?php base_url(); ?>assets/img/core-img/icon1.png" style="width:20px;height:20px">
                            <span class="translatable">Masih Berlaku</span>
                         </div>
					</div>
                                                ...
                                                <td  style="border-top:0px;">
                                                        ...
                                                        <input type="hidden" id="abstrak_<?php echo $nomor; ?>" value="<?php echo $hproduk['file_abstrak']; ?>">
															...
															<div style="margin-top:5px;" >
																...
																<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#modal_abstrak<?php echo $nomor; ?>" style="font-size:12px"><i class="fa fa-eye"></i> Abstrak</a> | 
																<?php
						...
						<div class="row">
							...
							<div class="card mb-2">
							  <img class="card-img-top"
								src="<?php echo base_url()?>/internal/assets/assets/berita/<?php echo $rberita['gambar_1'];?>" alt="Card image cap" style="width:100%;object-fit:cover">
							  <div class="card-body">
								<strong><?php echo $rberita['judul']?></strong>
								<p class="card-text">
									<?php echo substr($rberita['isi'],0,100)."...";?>
								</p>
								<a href="<?php echo base_url()?>berita/detail/<?php echo $rberita['id'];?>" class="btn btn-primary" style="color:#fff;">Detail</a>
							  </div>
							</div>
						  </div>
							<?php
...
<?php
if (sizeof($get_awal_produk) > 0) {
    $angka = 0;
    foreach ($get_awal_produk as $gap) {
        $angka++;
        $datakategori = $this->Mdl_home->get_kategori($gap['peraturan_category_id']);
        foreach ($datakategori as $dk) {
            $kodekat = $dk['percategorycode'];
        }
        $tahun = substr($gap['tanggal'], 0, 4);
        $bulan = substr($gap['tanggal'], 4, 2);
        ?>
        <div id="modal_abstrak<?php echo $angka ?>" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg" style="width:90%">
                ...
                <div class="modal-content" >
                    ...
                    <div class="modal-body">
                        <?php
                        if (!empty($gap['file_abstrak'])) {
                            ?>
                            <iframe src="<?php echo site_url('internal/assets/plugins/pdfjs/web/viewer.html?file=') . base_url('internal/assets/assets/produk_abstrak/' . $kodekat . '/' . $tahun . '/' . $bulan . '/' . $gap['file_abstrak']); ?>" style="width:100%; height:600px;" frameborder="0"></iframe>
                            <?php
                        } else {
                            echo 'Tidak ada file abstrak';
                        }
                        ?>
...
<script type="text/javascript">
	...
	function get_pdf(urut) {
        $('#modal_abstrak').modal({backdrop: 'static', keyboard: false});
        $("#div_frame").empty();
